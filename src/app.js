...
function renderHome() {
  ...
  const smgrEl = document.getElementById('setupManagers');
  const srndEl = document.getElementById('setupRounds');
  if (snEl && !snEl.dataset.dirty) snEl.value = state.leagueName || '';
  if (smgrEl && !smgrEl.dataset.dirty) smgrEl.value = (state.managers || []).join(', ');
  if (srndEl && !srndEl.dataset.dirty) srndEl.value = state.rounds || 8;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('setupSaveBtn')?.addEventListener('click', () => {
    ...
    if (picksExist && !confirm('Creating a new draft order will clear all current picks. Continue?')) return;
    const name = document.getElementById('setupLeagueName')?.value.trim();
    const mgrs = document.getElementById('setupManagers')?.value.split(',').map(m => m.trim()).filter(Boolean);
    ...
    state.drafted = {};
    addActivity('Draft order created for ' + state.managers.length + ' managers');
    saveState();
    render();
    toast('Draft order created!', 'success');
  });

  // Setup inputs - mark dirty on change so render won't overwrite
  ['setupLeagueName', 'setupManagers', 'setupRounds'].forEach(id => {
